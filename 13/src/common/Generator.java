...
public class Generator<C> {
    ...
    private final List<expression.common.Node<C>> nullary = new ArrayList<>();
    private final List<expression.common.Op<Integer>> ops = new ArrayList<>();
    ...
    public void add(final String name, final int arity) {
        if (arity == 0) {
            nullary.add(expression.common.Node.op(name));
        } else {
            ops.add(expression.common.Op.of(expression.common.Node.id(name, arity), arity));
        }
    }

    private expression.common.Node<C> generate(final boolean nullary, final IntFunction<Stream<expression.common.Node<C>>> children) {
        if (nullary || ops.isEmpty()) {
            return random.nextBoolean() ? random.randomItem(this.nullary) : expression.common.Node.constant(constant.get());
        } else {
            final Op<Integer> op = random.randomItem(ops);
            return expression.common.Node.op(op.name, children.apply(op.value).limit(op.value).collect(Collectors.toUnmodifiableList()));
        }
    }

    private expression.common.Node<C> generateFullDepth(final int depth) {
        return generate(depth == 0, arity -> Stream.generate(() -> generateFullDepth(depth - 1)));
    }

    private expression.common.Node<C> generatePartialDepth(final int depth) {
        return generate(depth == 0, arity -> Stream.generate(() -> generatePartialDepth(random.nextInt(depth))));
    }

    private expression.common.Node<C> generateSize(final int size) {
    ...
    public void testRandom(final int denominator, final TestCounter counter, final Consumer<expression.common.Node<C>> consumer) {
    ...
    private static <C> void testRandom(
            ...
            final Consumer<expression.common.Node<C>> consumer,
            final int seq,
            final int levels,
            final int perLevel,
